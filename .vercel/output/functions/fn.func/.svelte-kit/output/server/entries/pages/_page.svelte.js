...
const css = {
  code: "body{background-color:#f3f4f6}body.dark-mode{background-color:#1d3040;color:#bfc2c7}.svelte-85xtov:img-container{width:512;height:512}",
  map: null
};
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  const configuration = new Configuration({
    ...
    apiKey: "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"
  });
  new OpenAIApi(configuration);
  $$result.css.add(css);
  return `<div class="svelte-85xtov">${validate_component(Button, "Button").$$render($$result, {}, {}, {
    default: () => {
      return `Dark mode`;
    }
  })}
  <h1 class="text-3xl font-bold text-[#57534e] text-center my-7 svelte-85xtov">Image Generator
  </h1></div>

<div class="svelte-85xtov"><div class="flex justify-center items-center w-full svelte-85xtov"><input type="text" placeholder="Enter prompt here..."${add_attribute("value", prompt, 0)} class="block rounded-md w-[512px] border-0 py-1.5 ml-4 font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6 svelte-85xtov">
    <button type="submit" class="rounded-md text-[#f0ebe6] font-semibold py-2 px-8 ml-5 mr-8 bg-orange-500 svelte-85xtov">Generate</button></div>
  
  <div class="flex justify-center items-center pt-6 rounded-md img-container svelte-85xtov">${`<img class="rounded-md m-1 svelte-85xtov"${add_attribute("src", image, 0)} alt="">`}</div>
</div>`;
});
export {
  Page as default
